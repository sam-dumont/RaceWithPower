...
try:
    release_notes = requests.get(
        f"https://api.github.com/repos/samueldumont/RaceWithPower/releases/tags/{TAG_NAME}"
    ).json()["body"]
except:
    release_notes = ""
...
m = MultipartEncoder(
    fields={
        ...
        "submit": "",
        "file": (
            f"RaceWithPower-{TAG_NAME}.iq",
            open(f"/tmp/RaceWithPower-{TAG_NAME}.iq", "rb"),
...
m = MultipartEncoder(
    fields=[
        ...
        ("videoUrl", ""),
        ("devEmail", DEV_EMAIL),
        ("sourceUrl", "https://github.com/samueldumont/RaceWithPower"),
